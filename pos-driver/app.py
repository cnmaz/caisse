...
def debit(amount):
    transaction_start(amount)
    return f"<p>Debit f{float(amount)+1}</p>"
...
def prepare_data_to_send(amount):
...
def transaction_start(amount):
    ...
    try:
        ...
        if initialize_msg(serial):
            data = prepare_data_to_send(amount)
            if not data:
                return
